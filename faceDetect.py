...
def DetectFace(frame):
    ...
    if (len(faces) != 1):
        ...
        input("What to do? Execution resumes after input.")
...
def DetectFaceInList(frameList, faceCascade = None, debug = False):
	...
	print("该视频总共检测出",faceList.shape[0],"个face_in_list(cv2)文件")
	return faceList
...
def DetectFaceLandmarksInList(frameList, faceDetector = None, shapePredictor = None, skipLength = 2, debug = False):
	...
	faceList = np.array(faceList)
	print("该视频总共检测出",faceList.shape[0],"个face_landmark_in_list(dlib)文件")
	return faceList
...
def DetectFaceInListDlib(frameList, faceDetector = None, skipLength = 2, debug = False):
	...
	for i in range(len(rowList)):
		rowAvg = np.mean(rowList[int(max(0,i-winSize)):int(min(len(rowList),i+winSize))]) + 6
		colAvg = np.mean(colList[int(max(0,i-winSize)):int(min(len(colList),i+winSize))]) + 6
	...
	for i in range(len(detsList)):
		...
		faceImg = grayFrame[int(max(0,colc-(cols/2))):int(min(frame.shape[0],colc+(cols/2)+1)),int(max(0,rowc-(rows/2))):int(min(frame.shape[1],rowc+(rows/2)+1))]
		...
		if debug:
			win.clear_overlay()
			grayFrame[int(max(0,colc-(cols/2))):int(min(frame.shape[0],colc+(cols/2)+1)),int(max(0,rowc-(rows/2))):int(min(frame.shape[1],rowc+(rows/2)+1))] = faceImg
